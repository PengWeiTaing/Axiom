...
{snippet("core/receiver.py", 399, 431)}

## 原子写入

文本和图片都先写临时文件，再替换为正式文件，减少半截文件进入 inbox 的概率。

{snippet("core/receiver.py", 489, 516)}
""",
    )
...
{snippet("core/receiver.py", 870, 943)}

## 文本和图片写入

{snippet("core/receiver.py", 947, 1042)}

## 元数据、文件取回、归档和恢复

{snippet("core/receiver.py", 1045, 1079)}

{snippet("core/receiver.py", 1208, 1282)}
...
{snippet("core/receiver.py", 1285, 1362)}

{snippet("core/receiver.py", 1365, 1505)}

## 自动化产物读取

{snippet("core/receiver.py", 612, 733)}

{snippet("core/receiver.py", 736, 845)}

{snippet("core/receiver.py", 1082, 1205)}
...
{snippet("scripts/smoke_test_receiver.py", 433, 592)}
""",
    )
