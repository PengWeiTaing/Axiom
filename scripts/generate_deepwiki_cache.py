...
当前数据层的核心判断：文件系统保存内容本体，SQLite 保存索引。这个策略已经覆盖文本、图片、inbox、archive、备份、一致性检查和当前 Web App。
...
{snippet("core/receiver.py", 1250, 1321)}

## 文本和图片写入

{snippet("core/receiver.py", 1324, 1419)}

## 元数据、编辑、文件取回、归档和恢复

{snippet("core/receiver.py", 1422, 1432)}

{snippet("core/receiver.py", 1518, 1610)}

{snippet("core/receiver.py", 1613, 1634)}

{snippet("core/receiver.py", 1763, 1837)}

{snippet("core/receiver.py", 1143, 1239)}

{snippet("core/receiver.py", 1435, 1515)}

## 手动自动化入口

`/automation/jobs` 让前端知道当前允许手动运行哪些安全任务，`/automation/run` 负责串行触发白名单脚本。

当前只开放 review、inbox report 和 dry-run 类任务，不直接暴露 destructive apply。
...
{snippet("core/receiver.py", 1840, 2060)}

{snippet("core/receiver.py", 1637, 1760)}

## 自动化产物读取

{snippet("core/receiver.py", 797, 873)}

{snippet("core/receiver.py", 893, 1033)}

{snippet("core/receiver.py", 1036, 1140)}
