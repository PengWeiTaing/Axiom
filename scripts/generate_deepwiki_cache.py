...
| `data/reviews/` | 自动化产物 | review、inbox report、action snapshots、history、audio-transcripts |
...
| `/automation/run` | POST | 手动生成 review / inbox report / dry-run artifact / audio transcript report |
...
当前开放 review、inbox report、dry-run 和 `audio_transcribe_day`。其中音频自动转写需要在运行环境里配置 OpenAI key；本地冒烟则通过 mock 模板走通这条链路。每次手动触发都会写入运行历史，保留状态、耗时、stdout/stderr 尾部和关联产物。

## 最近记录和检索

`/recent` 和 `/search` 支持分页、类型、存储区、来源、时间范围过滤。`/search` 还支持 `relevance`、`newest`、`oldest` 排序，并会同时匹配原文件名、文档抽取正文和音频转写文本。

`/artifacts` 负责把 `data/reviews` 里的自动化产物列出来，当前支持 `review`、`inbox`、`inbox-actions`、`inbox-action-history`、`audio-transcripts` 五组，并支持 `window`、`mode`、日期范围、分页和排序。
