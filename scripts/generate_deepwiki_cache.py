...
Axiom 当前已经有可运行的 VPS 后端基线。它能接收文本、图片、文档和音频，能落盘，能入库，能检索，能取回文件，能归档恢复，能备份，也能自动生成回顾、inbox 处理报告和 action history。
...
| `/add` | GET/POST | 写入文本 |
| `/upload` | POST | 上传图片、PDF、Word 或音频；`.pdf` 与 `.docx` 会自动抽取正文 |
| `/item/<id>` | GET | 读取单条元数据 |
| `/item/<id>/update` | POST | 更新单条内容与来源 |
...
| `/automation/run` | POST | 手动生成 review / inbox report / dry-run artifact |
...
`/app` 是当前正式前端壳，`/sw.js` 让这个入口具备 PWA 基础能力。当前 Web 端已经补上 PDF 预览、音频播放和 Word 正文预览。

{snippet("core/receiver.py", 1519, 1592)}

## 文本和文件写入

{snippet("core/receiver.py", 1593, 1690)}

## 元数据、编辑、文件取回、归档和恢复

{snippet("core/receiver.py", 1691, 1701)}

{snippet("core/receiver.py", 1926, 2247)}

{snippet("core/receiver.py", 780, 970)}

{snippet("core/receiver.py", 1407, 1517)}

{snippet("core/receiver.py", 1704, 1925)}

## 手动自动化入口

`/automation/jobs` 让前端知道当前允许手动运行哪些安全任务，`/automation/run` 负责串行触发白名单脚本，`/automation/runs` 负责回看最近运行结果。

当前只开放 review、inbox report 和 dry-run 类任务，不直接暴露 destructive apply；每次手动触发都会写入运行历史，保留状态、耗时、stdout/stderr 尾部和关联产物。
...
{snippet("core/receiver.py", 2248, 2475)}

{snippet("core/receiver.py", 2045, 2247)}

## 自动化产物读取

{snippet("core/receiver.py", 1061, 1137)}

{snippet("core/receiver.py", 1231, 1297)}

{snippet("core/receiver.py", 1299, 1404)}
...
{snippet("docs/ITERATION_LOG.md", 160, 270, "markdown")}
""",
    )
