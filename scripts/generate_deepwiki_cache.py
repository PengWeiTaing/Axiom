...
| `/recent` | GET | 分页读取最近记录 |
| `/search` | GET | 关键词检索 |
| `/overview` | GET | 聚合返回统计、最近记录、待处理积压和最新自动化产物摘要 |
| `/overview/text` | GET | 返回适合快捷指令直接显示的纯文本总览 |
| `/processing/backlog` | GET | 按类型返回待补正文、待补转写、待补说明的积压条目，并附带下一条待处理记录 |
| `/processing/next` | GET | 返回当前下一条待处理记录，可选按类型过滤 |
...
`/processing/backlog` 会把待补正文、待补转写和待补说明的条目按类型聚合出来，并附上适合前端直接应用的快速过滤参数与 `next_item`。

`/processing/next` 提供统一的“下一条待处理记录”直达入口，前端可以直接打开 viewer，不必先跳到 recent 列表再二次点击。
