#!/usr/bin/env python3
"""
Backfill extracted text for existing PDF and DOCX items.

This script is meant for document items that were uploaded before
`derived_text` support was added, or for cases where extraction needs to be
rerun after fixing the extractor.
"""
...
from core.receiver import ITEM_TYPE_DOCUMENT, extract_document_text, init_db  # noqa: E402
...
def build_query(item_ids: list[int] | None, limit: int | None) -> tuple[str, list[object]]:
    query = """
        SELECT id, file_path, original_name, mime_type, derived_text
        FROM items
        WHERE type = ? AND file_path IS NOT NULL AND file_path != ''
        ORDER BY id
    """
    params: list[object] = [ITEM_TYPE_DOCUMENT]
...
def detect_extension(file_path: Path, original_name: str | None) -> str:
    if original_name:
        name_extension = Path(original_name).suffix.lower()
        if name_extension:
...
def backfill_document_text(
    ...
    try:
        ...
        for row in rows:
            summary["scanned"] += 1
            current_text = (row["derived_text"] or "").strip()
            ...
            extension = detect_extension(resolved_path, row["original_name"])
            if extension not in SUPPORTED_TEXT_EXTENSIONS:
                summary["skipped_unsupported"] += 1
                continue

            try:
                extracted_text = extract_document_text(resolved_path, row["original_name"])
            ...
            conn.execute(
                "UPDATE items SET derived_text = ? WHERE id = ?",
                (extracted_text, row["id"]),
            )
            summary["updated"] += 1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Backfill derived_text for existing PDF and DOCX items.",
    )
    parser.add_argument(
        "--root",
        type=Path,
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Re-extract even when derived_text already exists.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> None:
    args = parse_args()
    summary = backfill_document_text(
        root=args.root,
