...
**白板要允许认识被重构，而不是一键生成更多内容。**

Axiom 白板是从此刻或 Atlas 按问题展开的共同发现表面：用户可以先含混表达，AI 默认旁观并逐级追问、挑战或建立候选分支；只有经审查的主张、模型、证据与未知才能返回 Atlas。火山杯 Learning Board 是独立竞赛项目，不定义这一产品形态。

**界面是一件认知器物，不是功能仪表盘。**

Axiom 采用原生暗色的“夜间认知场”：石墨黑不是浅色界面的反转，而是让注意力从环境噪声中退场的基础材质；温暖白字、克制矿物色、连续分隔和轻微空间深度共同组织信息。此刻只留下一个可以开始的焦点，资料库是一册输入即查的连续索引，Atlas 则把同一片暗场展开为可远观、可聚焦的关系空间。完整方向见 [`docs/FRONTEND_ART_DIRECTION.md`](docs/FRONTEND_ART_DIRECTION.md)。

界面默认追求低摩擦：打开就是“此刻”，记录不要求预先分类，输入两个字符后资料库自动检索，任务、记忆、决定和治理能力只在需要时从索引或上下文出现。
