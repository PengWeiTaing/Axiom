...
- `AppNavigation.vue` 提供桌面顶部章节索引与移动底栏，只呈现三个一级目的地、全局记录和次级“索引”入口；`N` 直接记录，`/` 或 `Ctrl/Cmd+K` 直接查找。
...
- `DecisionsView.vue` 是 Vue 主线决策台，读取 `/decisions`，支持快速新增、状态筛选、填写实际结果并标记已回顾；决策列表筛选会同步到 URL，并支持从 URL 恢复筛选状态；列表项可用 `ObjectDrawer.vue` 查看完整决策详情。
- `AutomationView.vue` 是 Vue 主线自动化中心，读取 `/automation/jobs` 和 `/automation/runs`，支持运行日期、手动触发、状态/任务过滤、运行详情和 Markdown 产物预览；运行记录筛选会同步到 URL，并支持从 URL 恢复筛选状态。
- `SystemView.vue` 是 Vue 主线系统治理台，读取 `/system`、`/metrics`、`/audit-log` 和 `/admin/logs`，提供健康分、数据表、完整性、审计、日志尾部和数据导出；审计对象与日志等级筛选会同步到 URL，并支持从 URL 恢复筛选状态。
