...
```text
随时记录 → 此刻推动行动 → 资料库找回 → Atlas 理解关系 → 结果与复盘校准
```

当前阶段核心方向：
- 稳住“此刻 / 资料库 / Atlas”三个一级目的地，防止对象类型重新膨胀为工作台。
- 承诺档案、项目/生活线详情、行动结果、本周承诺、一层任务拆解与周复盘已经形成第一条完整闭环，当前转入真实使用校准。
- Atlas 保持 3D 全局、2D 聚焦的产品职责，并继续增强真实关系编辑与解释。
...
- “此刻”中的周复盘会保存用户对步骤粒度的判断，并作为下一次拆解建议的有限上下文；还没有形成跨场景、持续运行的主动推理层。
...
- 承诺闭环、项目/生活线详情、周承诺、一层任务拆解、AI 可撤回候选和周复盘已打通；继续用真实使用数据验证接受率、修改率、步骤粒度和完成反馈。
- 部署并观察自动生产状态快照（每日 system-status 报告，含 /health /system /stats /metrics 摘要）。
- 旧前端只迁移主产品流程真正需要的能力，并编排进现有三个目的地；不再按对象类型重建旧工作台。
- AI 层向主动推理过渡：只在明确高摩擦、证据充分且可以忽略的时刻给出有界提示，先验证站内提示，再考虑定时推送。
...
1. 用真实任务观察 AI 候选的接受、修改、放弃和完成情况，校准 V4 Pro 的拆解粒度与证据阈值。
2. 只在“行动长期未启动、步骤连续卡住或本周承诺明显失速”等高摩擦时刻试做可忽略的站内提示；不自动创建行动，不先做泛化推送中心。
3. 收集足够反馈后再决定是否需要依赖关系或更深层级；当前不扩张成任务树工作台。
