...
中枢的长期目的以 `docs/AXIOM_DISCOVERY_CORTEX_RESEARCH_V2.md` 为事实源：Axiom 不只理解与维护用户，还应自主观察世界、选择问题、寻找跨域规律、生成可证伪假说、设计验证并发现机会。各认知器官的产品研究入口是 `docs/discovery_cortex/00_RESEARCH_MAP.md`，其中 Atlas 以 `07_ATLAS_DISCOVERY_LANDSCAPE.md`、Axiom 白板以 `08_WHITEBOARD_CO_DISCOVERY_ENVIRONMENT.md`、二者交换以 `09_ATLAS_WHITEBOARD_CONTINUUM.md` 为下一代事实源。`AXIOM_CORTEX_AUTONOMY_RESEARCH_V1.md` 继续约束权限、审计、回滚和双环境隔离，但治理是发现的保护边界，不是自主性的终点。

Atlas 与白板必须严格区分人类认识界面和中枢内部结构：Atlas 只投影领域、地标、桥梁、张力、前沿与可审理主张；白板由 Axiom 围绕当前问题生成，把材料、机制、证据、反证、时间和情景编排成可理解、可质疑的展演。用户负责追问与判断，不负责拖动、连线或维护模型。白板布局不直接返回 Atlas，只有经过来源、边界和竞争解释审理的认识状态变化才可能更新长期地形。Axiom 白板不是第四个一级入口；火山杯 Learning Board 是独立竞赛项目，可借用基础能力但不能定义 Axiom 白板。

当前前端视觉人格以 `docs/FRONTEND_ART_DIRECTION.md` 为事实源：下一代采用原生暗色的“夜行长卷”，以连续游观、局部显影、空间尺度、克制光感和认识论视觉语法形成同一认知媒介；Atlas 是长期认识地形，白板是 Axiom 生成的动态认知展演。艺术感不来自卡片堆叠、浅色反转、蓝紫科技皮肤、行星节点或全局持续动画。

视觉状态需谨慎区分：上述文档已经锁定产品关系与视觉约束，但没有锁定最终艺术风格。2026-09-05 的第一张暗色概念图已被用户否决且未入库；后续不得把该图、星图式关系场或“夜行长卷”字面意象当成实现规格，应重新探索构图、材质、字体与空间人格。

低摩擦是产品约束而不是可选模式：默认进入“此刻”，全局记录不预先分类，资料库沿当前问题自然找回，次级对象只从当前上下文投影；自动化、结构、日志和系统治理进入独立 Foundry，不再塞入日常索引。新增能力不得以增加一级入口作为默认实现。
...
```text
frontend/
  src/                 # 当前可运行 Vue 主前端；下一代按“夜行长卷”整体重构
core/
  receiver.py
...
scripts/
  backup_axiom.py
  check_frontend_art_direction.py # 防止原生暗色主前端退回旧视觉语言
