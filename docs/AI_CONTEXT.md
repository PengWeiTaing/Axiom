...
Atlas 与白板必须严格区分人类认识界面和中枢内部结构：Atlas 只投影领域、地标、桥梁、张力、前沿与可审理主张；白板允许含混空间关系和 AI 候选，但只有经用户审查、明确认识地位与来源的成果才能晋升 Atlas。Axiom 白板不是第四个一级入口；火山杯 Learning Board 是独立竞赛项目，可借用能力但不能定义 Axiom 白板。

当前前端视觉人格以 `docs/FRONTEND_ART_DIRECTION.md` 为事实源：采用原生暗色的“夜间认知场”，以石墨材质、温暖白字、克制矿物色、连续索引和精确排印形成同一认知表面；Atlas 是这片暗场的关系空间，而不是另一套主题。艺术感不来自卡片堆叠、浅色反转、蓝紫科技皮肤、行星节点或全局持续动画。

低摩擦是产品约束而不是可选模式：默认进入“此刻”，全局记录不预先分类，资料库短查询自动检索，次级能力只从索引或当前上下文按需出现。新增能力不得以增加一级入口作为默认实现。
...
```text
frontend/
  src/                 # 当前主前端：夜间认知场 + 此刻 / 资料库 / Atlas + 全局记录
core/
  receiver.py
