...
core/
  ...
  context_commitments.py # 确认目标、目标行动和承诺断点
...
- `/api/context/now` 使用 `context.now.v3` 契约，根据截止压力、显式优先级、预估启动成本、生活线近期活动、搁置时长、近期显式反馈，以及行动与确认目标之间的显式关联给出稳定排序，同时返回判断理由、因子和承诺摘要
- 只有 `category=goal AND status=confirmed` 的记忆是承诺；candidate / archived 目标不参与排序。目标贡献是有上限的辅助因子，明确期限仍优先
- 确认目标没有 `todo` 行动时会进入 `commitments.gaps`；前端从“此刻”或目标详情补下一步时，新任务通过 `memory_id` 关联目标并继承目标 `lifeline_id`
- 推荐完成会写入 `context_action_outcomes`，保留推荐快照与任务结果；显式反馈只在 7 天窗口内衰减生效，紧迫期限优先，没有时长或生活线依据时不得跨任务外推
- `/export` 会包含 `context_action_outcomes.json`，行动结果和反馈与其他用户数据一样可导出；目标记忆和关联行动导入时保留 `lifeline_id`
