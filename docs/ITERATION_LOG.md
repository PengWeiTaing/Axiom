...
- 定义 Axiom 白板为共同发现环境：允许原生痕迹、涌现结构和正式模型共存，AI 默认旁观并以候选层逐级追问、挑战、预演和建立分支；核心能力是空间认识行动、渐进形式化和可逆表征转换，而不是模板库或批量生成。
- 明确火山杯 Learning Board 与 Axiom 白板的边界：前者继续独立竞赛交付，后者从“此刻”或 Atlas 当前问题按需展开，不成为第四个一级工作台；可复用画布能力，但产品本体、数据、导航和晋升治理独立。
- 建立 Atlas 与白板的双向契约：Atlas 的关系是可审理主张，白板的线可以只是试探；只有具备认识地位、来源、证据与边界的产物才能晋升，临时空间、反事实和未审 AI 候选不得污染长期地形。
