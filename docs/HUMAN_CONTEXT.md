...
1. `core/receiver.py`
   需要掌握：配置区、`init_db()`、`build_item_payload()`、`build_stats_payload()`、`build_artifact_payload()`、`build_artifact_latest_summary()`、`write_text_file_atomic()`、`write_binary_file_atomic()`、`insert_item()`、`overview()`、`get_item_file()`、`list_artifacts()`、`artifact_summary()`、`get_artifact_file()`、`archive_item()`、`restore_item()`、`recent_items()`、`search_items()`。
