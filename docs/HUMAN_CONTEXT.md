...
- “此刻”已经能从真实任务事实中选出一个主要行动，解释为什么，并在完成后自动重排；完成后的可选轻反馈只短期校准相近时长或生活线，不会被固化成永久画像，也不假装掌握尚未记录的精力、目标贡献或影响后果。
...
3. `core/context_engine.py` 与 `core/routes/context.py`
   需要掌握：此刻如何只基于已知事实给出可解释判断，如何区分完成结果与显式反馈，以及 `context.now.v2` 契约。
...
- 真实数据是否始终可备份、可恢复、可校验。
- “此刻”的判断是否只基于可核对事实，并能通过完成、改期和反馈逐步校准。
