...
- 主前端采用“墨与光”视觉人格：此刻是行动长卷，资料库是生长中的索引，Atlas 是全屏认知场。完整规则见 `docs/FRONTEND_ART_DIRECTION.md`，后续页面不能退回卡片墙或科技仪表盘。
...
2. `docs/FRONTEND_ART_DIRECTION.md`
   需要掌握：“墨与光”如何把留白、语义色、状态光和点线关系转化为统一产品感知，以及哪些旧视觉模式不得返回。
3. `frontend/src/`
