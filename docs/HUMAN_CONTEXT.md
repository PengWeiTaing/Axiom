...
- 主前端采用“活页”视觉人格：此刻是单焦点行动页，资料库是输入即查的连续索引，Atlas 是按需进入的深色认知场。日常路径默认低摩擦，次级对象只从索引或当前上下文出现。完整规则见 `docs/FRONTEND_ART_DIRECTION.md`，后续页面不能退回卡片墙或科技仪表盘。
...
2. `docs/FRONTEND_ART_DIRECTION.md`
   需要掌握：“活页”如何把编辑秩序、连续纸面、语义色和点线关系转化为统一产品感知，低摩擦如何约束入口与操作，以及哪些旧视觉模式不得返回。
3. `frontend/src/`
