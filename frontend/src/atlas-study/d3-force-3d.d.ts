...
declare module 'd3-force-3d' {
  ...
  export interface PositionForce extends Force { strength(value: number): this; }
  ...
  export function forceX<N>(target: (node: N) => number): PositionForce;
  export function forceY<N>(target: (node: N) => number): PositionForce;
  export function forceZ<N>(target: (node: N) => number): PositionForce;
}
